...
def create_perc_fig(df, group_column):
    ...
    df = (
        ...
        .reset_index()
        .melt(id_vars="Month_Year", var_name=group_column, value_name="Percentage")
    )
    ...
    fig = px.bar(
        ...
        y="Percentage",
        color=group_column,
        title=f"Evolution of Sales by {group_column} over Time",
...
def on_selector(state):
    filtered_data = state.data.loc[state.data["City"].isin(state.city)]
